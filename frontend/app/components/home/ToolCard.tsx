...
interface ToolCardProps {
    tool: Tools,
    onUpvote: (toolId: string, newCount: number) => void,
...
export default function ToolCard({ tool, onUpvote, userEmail, index = 0 }: ToolCardProps) {
    ...
    const handleUpvote = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        setIsUpvoting(true);
        try {
            // 1. Chamada para o seu endpoint Java (ex: POST /api/resources/{id}/upvote)
            // O backend vai verificar se já existe o upvote: 
            // - Se existe, ele deleta (downvote)
            // - Se não existe, ele cria (upvote)
            // const response = await api.post(`/resources/${tool.id}/upvote`);

            // 2. O backend deve retornar o novo total e o status
            // const { upvotesCount, votedByMe } = response.data;

            // 3. Atualiza o estado local com o que o servidor confirmou
            // setLocalUpvotes(upvotesCount);
            // setHasUpvoted(votedByMe);

            // if (onUpvote) onUpvote(tool.id, upvotesCount);
        } catch (error) {
